...
public class CodeFetchStage {
    ...
    public void execute(ReviewContext context) throws Exception {
        ...
        if (config.repoAppUrl == null || config.repoAppUrl.isBlank()) {
        ...
        String branch = config.repoAppBranch != null ? config.repoAppBranch : "main";
        String token = gitHubService.resolveToken(config.repoAppUrl);
        logger.info("Fetching app repo code for service {} from {} branch {}", config.serviceId, config.repoAppUrl, branch);
        // Get the latest commit SHA
        String sha = gitHubService.getLatestCommitSha(config.repoAppUrl, branch, token);
        if (sha != null) {
            context.repoShas.put("app", sha);
        }
        // Fetch source files
        String dirPath = config.repoAppServicePath != null ? config.repoAppServicePath : "";
        String codeText = gitHubService.fetchDirectoryAsText(
                config.repoAppUrl, branch, dirPath,
                config.repoAppIncludePaths, config.repoAppExcludePaths, token);
